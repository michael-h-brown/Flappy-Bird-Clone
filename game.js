...
function gameAI() {
		...
		for (var i = 0; i < ais.length; i += 1) {
			if (ais[i].dead == false) {
				...
				if (hasCollided == 2) {
					ais[i].score += 1;
					ais[i].passedWall();
					if (ais[i].score > aiHighScore) {
						aiHighScore = ais[i].score;
					}
					if (ais[i].score > currGenAIHighScore) {
						currGenAIHighScore = ais[i].score;
					}
				} else if (hasCollided == 1) {
					ais[i].die();
				}
	...
	aliveAICount = aiCount;

	if (allDead) {
		currGenAIHighScore = 0;
		generation += 1;
		nextGen();
		spawner = new WallSpawner();
		handler.nextAIState();
	...
	ctx.fillText('Alive AIs: ' + aliveAICount.toString() + "/" + noOfAI.toString(), canvas.width / 2.5 + 100, 20);
	ctx.fillText('CurrGen High Score: ' + currGenAIHighScore.toString(), canvas.width /2.5, 40)
	ctx.fillText('High Score: ' + aiHighScore.toString(), canvas.width / 2.5, 60);
}
